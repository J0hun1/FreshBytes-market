          ...
          <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
              <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-gray-500 dark:text-gray-400 md:ms-2">Vegetables</span>
              </div>
            </li>
          </ol>
        </nav>
        <h2 class="mt-3 text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">Leafy Greens</h2>
      </div>
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <div class="h-56 w-full">
          <a href="{{ route('product.show', $product->product_id) }}">
            <img class="mx-auto h-full dark:hidden" src="{{ $product->image_light ?? 'https://flowbite.s3.amazonaws.com/blocks/e-commerce/imac-front.svg' }}" alt="{{ $product->product_name }}" />
            <img class="mx-auto hidden h-full dark:block" src="{{ $product->image_dark ?? 'https://flowbite.s3.amazonaws.com/blocks/e-commerce/imac-front-dark.svg' }}" alt="{{ $product->product_name }}" />
          </a>
        </div>
        <div class="pt-6">
          <div class="mb-4 flex items-center justify-between gap-4">
            @if($product->is_discounted)
            <span class="me-2 rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-primary-800 dark:bg-primary-900 dark:text-primary-300"> Up to {{ $product->discounted_amount }}% off </span>
            @endif

            <div class="flex items-center justify-end gap-1">
              <button type="button" data-tooltip-target="tooltip-quick-look-{{ $product->product_id }}" class="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
              ...
              <div id="tooltip-quick-look-{{ $product->product_id }}" role="tooltip" class="tooltip invisible absolute z-10 inline-block rounded-lg bg-gray-900 px-3 py-2 text-sm font-medium text-white opacity-0 shadow-sm transition-opacity duration-300 dark:bg-gray-700" data-popper-placement="top">
                Quick look
                <div class="tooltip-arrow" data-popper-arrow=""></div>
              </div>

              <button type="button" data-tooltip-target="tooltip-add-to-favorites-{{ $product->product_id }}" class="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                <span class="sr-only"> Add to Favorites </span>
                <svg class="h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6C6.5 1 1 8 5.8 13l6.2 7 6.2-7C23 8 17.5 1 12 6Z" />
                </svg>
              </button>
              <div id="tooltip-add-to-favorites-{{ $product->product_id }}" role="tooltip" class="tooltip invisible absolute z-10 inline-block rounded-lg bg-gray-900 px-3 py-2 text-sm font-medium text-white opacity-0 shadow-sm transition-opacity duration-300 dark:bg-gray-700" data-popper-placement="top">
                Add to favorites
                <div class="tooltip-arrow" data-popper-arrow=""></div>
              </div>
            </div>
          ...
          <a href="{{ route('product.show', $product->product_id) }}" class="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{{ $product->product_name }}</a>
          ...
          <div class="mt-2 flex items-center gap-2">
            ...
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $product->top_rated ? 5.0 : 4.8 }}</p>
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">({{ $product->sell_count }})</p>
          </div>
...
<div class="mt-4 flex items-center justify-between gap-4">
            <p class="text-2xl font-extrabold leading-tight text-gray-900 dark:text-white">₱{{ number_format($product->product_price, 2) }}</p>

            <form action="{{ route('cart.add', $product->product_id) }}" method="POST" class="inline">
              @csrf
